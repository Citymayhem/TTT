...
if SERVER then
   ...
   concommand.Add("ttt_c4_destroy", ReceiveC4Destroy)
	--BCODE
	hook.Add("EntityRemoved", "c4_entRemoved", function(ent)
		if(ent:GetClass() == "ttt_c4")then
			if(!ent.IsOnPlayer)then return end
			ent.host.CarryingBomb = false
		end
	end)
	--BCODE
	function c4LostHost(ply)
		if(!ply.CarryingBomb)then return end
		for entid, ent in pairs(ents.FindByClass("ttt_c4")) do
			if(!ent.IsOnPlayer || ent.host!=ply)then continue end
			local pos = ent:GetPos()
			ent.IsOnPlayer = false
			ent:SetParent()
			ply.CarryingBomb = false
			ent.host = nil
	...
	hook.Add("PlayerDisconnected", "c4_hostDisconnected", c4LostHost)
	hook.Add("PlayerDeath", "c4_hostDeath", c4LostHost)--END OF BCODE
end
